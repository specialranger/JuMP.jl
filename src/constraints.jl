...
"""
    set_normalized_coefficient(constraint::ConstraintRef, variable::VariableRef, value)
...
`set_normalized_coefficient(c, x, 4)` will create the constraint `4x <= 2`.
...
"""
    normalized_coefficient(constraint::ConstraintRef, variable::VariableRef)
...
2`, `set_normalized_rhs(c, 4)` will create the constraint `2x <= 4`, not `2x +
1 <= 4`.
...
constraint into its standard form. See also [`set_normalized_rhs`](@ref).
"""
